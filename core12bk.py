...
def main():
    ...
    with col1:    	
        ...
        strike_price = st.number_input("Strike Price *")  # Ensure non-negative
    with col2:
    ...
    with col3:
        ...
        to_date = st.date_input("To Date *", format="DD-MM-YYYY")
        
    
    # Validation: Check if all fields are filled and valid
    all_fields_filled = (
        symbol is not None and
        instrument_type is not None and
        strike_price > 0 and  # Ensure strike price is positive
    ...
    if not all_fields_filled:
        st.warning("Please fill all required fields marked with * and ensure From Date is not after To Date.")
    
    # Disable button if fields are not filled
