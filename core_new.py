...
def calculate_from_date(to_date, duration):
    ...
    if duration == "1D":
        ...
        return to_date - timedelta(days=45)  # Approximate 1.5 months
    elif duration == "3M":
...
def main():
    ...
    with col2:        
        ...
        # Set default dates relative to current date (June 01, 2025)
        current_date = datetime(2025, 6, 1)
        default_to_date = current_date
        calculated_from_date = calculate_from_date(default_to_date, duration) if duration != "Custom" else default_to_date - timedelta(days=30)
        from_date = st.date_input("From Date *", value=calculated_from_date, format="DD-MM-YYYY", min_value=datetime(2000, 1, 1))
        # To Date must be after From Date, default to current date
        to_date = st.date_input("To Date *", value=default_to_date, min_value=from_date + timedelta(days=1), format="DD-MM-YYYY")
    
    # Column 3: Last 3 filters
    with col3:        
        expiry_date = st.date_input("Expiry Date (DD-MMM-YYYY) *", value=datetime(2025, 6, 26), format="DD-MM-YYYY", min_value=datetime(2000, 1, 1))
